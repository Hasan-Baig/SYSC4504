$(function () {
    ...
    var grid = $("main ul.gallery");
    var rows = 3;
    var cols = 5;

    var createLayout = function (rows, cols) {
        var count = 0;
        for (var i = 0; i < rows; i++) {
            grid.append('<li>');
            for (var j = 0; j < cols; j++) {
                grid.append('<img src="images/square/' + images[count].path + '" alt="' + images[count].title + '">');
                count++;
            }
            grid.append('</li>');
        }
    ...
    $("img")
        .on("mouseenter", function () {
            ...
            var img;
            for(var a in images) {
                if (images[a].title == $(this).attr("alt")){
                    img=images[a];
                }
            }
            var divPreview = $('<div id = "preview"><img src="images/medium/'+img.path+'" alt="'+img.title+'"><p>'+img.title+'</p><p><em>'+img.city+', '+img.country+' ['+img.taken+']</em></p></div>');
            
            $("body").append(divPreview);
